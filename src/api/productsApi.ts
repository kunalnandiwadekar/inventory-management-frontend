...
export const addProduct = async (product: {
  ...
  price: number; min_stock: number; current_stock: number;
}) => {
...
export const stockIn = async (id: number, quantity: number) => {
  const res = await fetch(`${BASE_URL}/products/${id}/stock-in`, {
    method: "POST",
    headers: authHeaders(),
...
export const stockOut = async (id: number, quantity: number) => {
  const res = await fetch(`${BASE_URL}/products/${id}/stock-out`, {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({ quantity }),
  });
